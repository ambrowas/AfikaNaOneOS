...
import Combine
    ...
    class JugarModoCompeticionViewModel: ObservableObject {
       // @Published var currentQuestion: String = ""
        ...
        @Published var shouldFlashConfirmButton: Bool = false
        @Published var confirmButtonScale: CGFloat = 1.0
    ...
    options.shuffled()
            ...
            buttonBackgroundColors = Array(repeating: Color(red: 88/255, green: 81/255, blue: 35/255), count: 3)
            ...
            if let countdownURL = Bundle.main.url(forResource: "countdown", withExtension: "wav"),
               ...
               let swooshURL = Bundle.main.url(forResource: "swoosh", withExtension: "wav") { // Add swoosh sound URL
                ...
                shouldFlashConfirmButton = true // Start flashing

                // Grow-Shrink Effect
                Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { timer in
                    DispatchQueue.main.async {
                        withAnimation(.easeInOut(duration: 0.5)) {
                            self.confirmButtonScale = self.confirmButtonScale == 1.0 ? 1.2 : 1.0
                        }
                    }

                    if !self.shouldFlashConfirmButton {
                        timer.invalidate() // Stop animation when flashing stops
                    }
                }

                // Stop flashing after 9 seconds
                DispatchQueue.main.asyncAfter(deadline: .now() + 9) {
                    self.shouldFlashConfirmButton = false
                    self.confirmButtonScale = 1.0 // Reset scaling
                }
            }
        
                func incrementMistakesAndHandleAlerts() {
                // Increment mistakes count
                mistakes += 1
                print("Mistakes updated to: \(mistakes)")

                // Call handleAlerts after updating the mistakes count
                DispatchQueue.main.async {
                    self.handleAlerts()
                }
            }
                ...
                if unusedQuestionsCount > 8 {
                    ...
                    DispatchQueue.main.async {
                           self.setButtons() // ✅ Ensure button colors reset
                       }
            }
       ...
       self.fetchQuestionBatch()
            ...
            questionManager?.fetchCurrentBatchForUser { [weak self] batchNumber in
                ...
                if let question = DatabaseManager.shared.fetchRandomQuestionFromLocalDatabase() {
                    DispatchQueue.main.async {
                        ...
                        self.selectedOptionIndex = nil // ✅ Reset previous selection
                        self.resetButtonColors() // ✅ Force reset button colors
                        self.startTimer()
                    }
                }
            }
            ...
            if let questionText = data?["QUESTION"] as? String,
               ...
               let number = data?["NUMBER"] as? String { // Assume "NUMBER" is the key for the question number
                ...
                if let selectedOptionIndex = selectedOptionIndex {
                    ...
                    shouldFlashConfirmButton = false // ✅ Stop flashing after confirming
                          confirmButtonScale = 1.0 //
                ...
                incrementMistakesAndHandleAlerts() // Replaces direct increment
                totalScore -= 500
                answerIsCorrect = false
                print("handleIncorrectAnswer - Incorrect answer selected.")

                // Reset and prepare for next question
                questionProcessed = true
                buttonText = "NEXT"
        ...
        if timeRemaining <= 5 && timeRemaining > 0 {
            playCountdownSound()
                    ...
                    if timeRemaining == 5 {
                               triggerVibration() // ✅ Vibrate at 5 seconds remaining
                           }
        ...
        if timeRemaining == 0 {
            ...
            timerIsActive = false
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    generator.impactOccurred() // ✅ Second vibration
                }
            }
                ...
                // Penalize the user for not answering in time
                print("Time expired. Penalizing user...")
                incrementMistakesAndHandleAlerts() // Increment mistakes and trigger alerts
                totalScore -= 500

                // Mark the answer as incorrect and notify the UI
                DispatchQueue.main.async {
                    self.answerIsCorrect = false // Signal incorrect answer
                    self.answerChecked = true    // Signal that the answer was checked
                }

                // Update button to proceed to the next question
                buttonText = "NEXT"
                questionProcessed = true

                // Notify the UI explicitly to refresh
                DispatchQueue.main.async {
            ...
            timerIsActive = false // Timer is no longer active
                ...
                // High-priority: "Game Over" Alert
                if mistakes >= 5 {
                    print("handleAlerts - Triggering 'Game Over' alert.")
                    triggerGameOverAlert()
                    return
                }

                // Medium-priority: "Too Many Mistakes" Alert
                if mistakes == 4 && !hasShownManyMistakesAlert {
                    print("handleAlerts - Triggering 'Too Many Mistakes' alert.")
                    triggerManyMistakesAlert()
                    hasShownManyMistakesAlert = true
                    return
                }

                // Low-priority: Other Alerts
                if activeAlert == .showReturnToAppAlert {
                    print("handleAlerts - Showing 'Return to App' alert.")
                    triggerReturnToAppAlert()
                    return
                }

                print("handleAlerts - No alerts to trigger.")
            }
               ...
               // playWarningSound()
                ...
                print("User left the app during an active question. Applying penalty...")
                playWrongSoundEffect()
                incrementMistakesAndHandleAlerts() // Replaces direct increment
                totalScore -= 500
        ...
        func setButtons() {
            ...
            DispatchQueue.main.async {
                // ✅ Correct Default Color: Olive Green (#585123)
                self.buttonBackgroundColors = Array(repeating: Color(red: 88/255, green: 81/255, blue: 35/255), count: self.options.count)
            ...
            DispatchQueue.main.async {
                ...
                for index in self.options.indices {
                    if index == self.selectedOptionIndex {
                        colors.append(Color(red: 84/255, green: 8/255, blue: 4/255)) // ✅ Dark Red for selection
                    } else {
                        colors.append(Color(red: 88/255, green: 81/255, blue: 35/255)) // ✅ Olive Green
                    }
                }
                self.buttonBackgroundColors = colors
                self.objectWillChange.send() // ✅ Force SwiftUI to re-render
            }
        }

        func resetButtonColors() {
            print("Resetting button colors to Olive Green") // Debug log
            buttonBackgroundColors = Array(repeating: Color(red: 88/255, green: 81/255, blue: 35/255), count: options.count)
        }
